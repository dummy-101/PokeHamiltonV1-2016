...
def get_args():
    ...
    parser.add_argument('-al', '--access-log', help="Write web logs to access.log.",
                        action='store_true', default=False)
    parser.add_argument('-st', '--step-limit', help='Steps', type=int,
                        default=12)
    parser.add_argument('-gf', '--geofence-file',
                        help=('Geofence file (currently only for HexSearch)'))
    parser.add_argument('-fa', '--forbidden-area',
                        help=('File with coordinates defining area NOT to scan. Basically this is inverted geofence. Can be combined with -gf'))
    ...
    parser.add_argument('-sl2', '--speed-limit',
    ...
    parser.add_argument('-sap', '--scout-account-password', default=None,
                        help='Scout password')
    parser.add_argument('-sapp', '--scout-account-proxy', default=None,
                        help='Scout proxy')
    parser.add_argument('-ditto', '--ditto',
                        ...
                        action='store_true', default=False)
    parser.add_argument('-pstop', '--doPstop',
                        help='Spin all Pokestops',
                        action='store_true', default=False)
    ...
    parser.add_argument('-egg', '--doEgg',
                        help='Hatch collected eggs',
                        action='store_true', default=False)
    parser.add_argument('-lc', '--level-cap',
                        help='Stop catching Ditto-mons, spinning stops and incubating eggs at this level.', default=40)
    ...
    encounter_list.add_argument('-ewhtf', '--encounter-whitelist-file',
                                default='', help='File containing a list of Pokemon to encounter for more stats.')
    encounter_list.add_argument('-eblkf', '--encounter-blacklist-file',
                                default='', help='File containing a list of Pokemon to NOT encounter for more stats.')
    ...
    parser.add_argument('-cri', '--captcha-rest-interval', type=int, default=600,
                        help='Seconds for accounts to rest when failing to verify captcha due to 2captcha server timeout. Defaults to 600 secs')
    ...
    parser.add_argument('-lr', '--login-retries',
                        help=('Number of times to retry the login before refreshing a thread.'),
                        type=int, default=3)
    ...
    parser.add_argument('-lp', '--lured-pokemon',
                        help=('Shows lured Pokemon on map (including parsing them into local db).'),
                        action='store_true', default=False)
    parser.add_argument('-ldur', '--lure-duration',
                        help=('Change duration for lures set on pokestops. This is useful for events that extend lure duration'), type=int, default=30)
    ...
    parser.add_argument('-vci', '--version-check-interval', type=int,
                        help='Interval to check API version force in secs (default: 300)',
                        default=300)
    ...
    parser.add_argument('--disable-blacklist', help=('Disable the global anti-scraper IP blacklist.'),
                        action='store_true', default=False)
...
def get_tutorial_state(api, account):
    log.warning('Checking tutorial state for %s.', account['username'])
...
def complete_tutorial(api, account, tutorial_state):
    if 0 not in tutorial_state:
        ...
        log.warning('Sending 0 tutorials_completed for %s.', account['username'])
        request.call()
    ...
    if 1 not in tutorial_state:
        ...
        log.warning('Sending set random player character request for %s.',
                  account['username'])
        request.call()
        ...
        log.warning('Sending 1 tutorials_completed for %s.', account['username'])
        request.call()

    time.sleep(random.uniform(0.5, 0.6))
    request = api.create_request()
    request.get_player_profile()
    log.warning('Fetching player profile for %s...', account['username'])
    request.call()
    ...
    if 3 not in tutorial_state:
        ...
        log.warning('Grabbing some game assets.')
        request.call()
        ...
        log.warning('Catching the starter for %s.', account['username'])
        request.call()
    ...
    if 4 not in tutorial_state:
        ...
        log.warning('Claiming codename for %s.', account['username'])
        request.call()
    ...
    if starter_id:
        ...
        log.warning('Setting buddy pokemon for %s.', account['username'])
        request.call()
        time.sleep(random.uniform(0.8, 1.8))

    # Sleeping before we start scanning to avoid Niantic throttling.
    log.warning('And %s is done. Wait for a second, to avoid throttle.',
              account['username'])
    time.sleep(random.uniform(2, 4))
    return True
...
def get_blacklist():
    try:
        ...
        log.warning('Entries in blacklist: %s.', len(blacklist))
        return blacklist
