...
class HexSearch(BaseScheduler):
    ...
    def __init__(self, queues, status, args):
        ...
        # If we are only scanning for pokestops/gyms, the scan radius can be 450m.  Otherwise 70m
        if self.args.scan_distance:
            self.step_distance = self.args.scan_distance
    ...
    def _generate_locations(self):
        ...
        if self.args.forbidden_area is not None:
            results = geofence(results, self.args.forbidden_area,
                               forbidden=True)
...
class SpawnScan(BaseScheduler):
    ...
    def _generate_locations(self):
        ...
        if self.args.spawnpoint_scanning != 'nofile':
            log.info('Loading spawn points from json file @ %s', self.args.spawnpoint_scanning)
            try:
 ...
 # ###################################################################################
        #if self.args.geofence_file is not None:
        #    scan_location = geofence(scan_location, self.args.geofence_file)
        #if self.args.forbidden_area is not None:
        #    scan_location = geofence(scan_location, self.args.forbidden_area,
        #                       forbidden=True)
        # Well shit...
        # if not self.locations:
