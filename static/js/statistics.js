...
function addElement (pokemonId, name) {
  ...
  jQuery('<a/>', {
    id: 'link_' + pokemonId,
    href: 'http://pokehamilton.com/pokemon/' + pokemonId,
    target: '_blank',
...
function processSeen (seen) {
  ...
  for (i = seen.pokemon.length - 1; i >= 0; i--) {
    ...
    $('#location_' + item['pokemon_id']).html('<strong>GPS:</strong> ' + location)
...
function processAppearance (i, item) {
  ...
  if (!((spawnpointId) in mapData.appearances)) {
    ...
    item['marker'] = setupPokemonMarkerHM(item, map, true)
...
function redrawAppearances (appearances) {
  $.each(appearances, function (key, value) {
    ...
    if (!item['hidden']) {
      var newMarker = setupPokemonMarkerHM(item, map, true)
      item['marker'].setMap(null)
